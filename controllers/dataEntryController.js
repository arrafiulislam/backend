...
const StandardizedValuesModel = require("../models/standardizedValuesModel.js");
const WeightedScoreModel = require("../models/weightedScoresModel.js");
const RowMeanModel = require("../models/rowMean.js");
const RowMeanSDModel = require("../models/sdofrowmean.js");
...
class DataController {
  static async createEntry(req, res) {
    try {
      ...
      if (!dateOfRecord) {
        return res
          .status(400)
          .send({ status: "failed", message: "dateOfRecord is required." });
      }
      ...
      if (existingEntry) {
        return res.status(400).send({
          status: "failed",
          message: "Data entry for the same date already exists.",
        });
      }
      ...
      res
        .status(500)
        .send({ status: "failed", message: "Unable to create data entry." });
    }
  }
  ...
  static async calculateMeanAndSD(userId) {
    try {
      // Fetch user data
      ...
      if (userData.length < 30) {
        ...
        return {
          status: "failed",
          message: "Insufficient data (less than 30 days).",
        };
      }
      ...
      fields.forEach((field) => {
        ...
        const variance =
          values.reduce(
            (sum, value) => sum + Math.pow(value - fieldMean, 2),
            0
          ) / values.length;
  ...
  static async calculateStandardizedValues(userId) {
    try {
      // Retrieve the latest statistics for the user
      const statistics = await StatisticsModel.findOne({ userId }).sort({
        updatedAt: -1,
      });
      if (!statistics) {
        ...
        return {
          status: "failed",
          message: "No statistics found for the user.",
        };
      }
      ...
      if (!plainMean || !plainSD) {
        ...
        return {
          status: "failed",
          message:
            "Mean or standard deviation is missing or invalid in statistics.",
        };
      }
      ...
      console.log(
        `Standardized values calculated and stored successfully for user: ${userId}`
      );
      return {
        status: "success",
        message: "Standardized values calculated and stored successfully.",
      };
    } catch (error) {
      console.error(
        `Error calculating standardized values for user: ${userId}`,
        error
      );
      return { status: "failed", message: "Internal server error." };
    }
  ...
  static async calculateWeightedScores(userId) {
    try {
      if (!userId) {
      ...
      const standardizedValuesEntries = await StandardizedValuesModel.find({
        userId,
      }).sort({ updatedAt: -1 });
      ...
      for (const entry of standardizedValuesEntries) {
        ...
        for (const field in weights) {
          ...
          if (standardizedValues instanceof Map) {
            ...
            standardizedValues &&
            standardizedValues[field] !== undefined
          ) {
      ...
      console.log(
        "Weighted scores calculated and stored successfully.",
        weightedScores
      );
  ...
  static async calculateRowMean(userId, selectedDate) {
    try {
      ...
      if (!dataEntries.length) {
        return {
          status: "failed",
          message:
            "No data entries found for the user within the last 30 days.",
        };
      }
  ...
  static async calculateSDofRM(userId, selectedDate) {
    try {
      // Calculate the 30-day range from the selected date
      const selectedDateObj = new Date(selectedDate);
      const startDate = new Date(selectedDateObj);
      ...
      if (rowMeans.length < 30) {
        return {
          status: "failed",
          message: "Insufficient data for SD calculation (less than 30 days).",
        };
      }
      ...
      return {
        status: "success",
        message:
          "Standard deviation of row means calculated and stored successfully.",
        data: { standardDeviation },
      };
    } catch (error) {
      console.error(
        "Error calculating standard deviation of row means:",
        error
      );
      return { status: "failed", message: "Internal server error." };
    }
  ...
  static async calculateFinalScore(userId, selectedDate = null) {
    try {
      ...
      if (selectedDate) {
        ...
        const latestData = await DataModel.find({ userId })
          .sort({ dateOfRecord: -1 })
          .limit(1);
      ...
      const standardizedValuesResult =
        await DataController.calculateStandardizedValues(userId);
      if (standardizedValuesResult.status === "failed") {
        console.log(
          "Error in calculateStandardizedValues:",
          standardizedValuesResult.message
        );
        return standardizedValuesResult; // Ensure to return the failed result with status
      }

      const weightedScoresResult = await DataController.calculateWeightedScores(
        userId
      );
      if (weightedScoresResult.status === "failed") {
        console.log(
          "Error in calculateWeightedScores:",
          weightedScoresResult.message
        );
        return weightedScoresResult; // Ensure to return the failed result with status
      }

      const rowMeanResult = await DataController.calculateRowMean(
        userId,
      ...
      const sdResult = await DataController.calculateSDofRM(
        userId,
        selectedDate || endDateStr
      );
      ...
      const finalScore = rowMeanResult.rowMeans.map((rowMean) => {
        ...
        const weightedScore =
          weightedScoresResult.data.find((ws) => ws.date === rowMean.date)
            ?.weightedScore || 0;
  ...
  static async getFinalScore(req, res) {
    try {
      const userId = req.user._id;
      ...
      if (selectedDate) {
        ...
        const latestEntry = await DataModel.findOne({ userId }).sort({
          dateOfRecord: -1,
        });
      ...
      const finalScoreResult = await DataController.calculateFinalScore(
        userId,
        selectedDateObj
      );
      ...
      if (finalScoreResult.status === "failed") {
        console.log(
          "Final Score Calculation Failed:",
